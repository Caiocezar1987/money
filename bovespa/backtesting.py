...
def calc_all_trends():
    ...
    for code in codes:
        ...
        if len(code_trend['trend']) > 24 and code_trend['trend'][-1] == 'buy':
            trade = Trade()
            trade.code = code
            trade.qtd = 0
            trade.begin = quote['date'][-1]
            trade.buy = quote['maxPrice'][-1] # usando pior preco para compensar slipage
            trade.volume = quote['volume'][-1] # pegando o volume do dia para quando calcular trades
            trade.sell = quote['minPrice'][-1] # usando pior preco para compensar slipage
            trade.stop = quote['minPrice'][-1]
            trade.stop2 = quote['minPrice'][-1]
            trade.end = quote['date'][-1]
            trade.result = trade.sell - trade.buy
            ret.append(trade)
...
class BackTesting:
    """Informacoes de um backtesting."""

    def __init__(self):
        self.equity = 100000
        self.risk = 0.005
        self.deposit = 50000
        self.minwageequity = 1000000
        self.jgrafixPath = r'C:\Tools\JGrafix\dados'
        self.tradesOutputFile = 'Trades.csv'
        self.moneyOutputFile = 'Money.csv'

    def run(self):
        trades = calc_all_trades()
        export_trades(trades, self.tradesOutputFile)
        return trades

    def makemoney(self, trades):
        self.money = calc_money(self.trades, self.equity, self.risk, self.deposit, self.calcwage(self.minwageequity))
        export_trades(self.trades, self.tradesOutputFile)
        export_money_to_csv(self.money, self.moneyOutputFile)

    def import_quote(self):
        import_quote_from_jgrafix(self.jgrafixPath)
...
def trades():
    ...
    for trade in lasttrades:
        print str(trade.begin) + ' ' + trade.code + ' @' + str('%.2f' % trade.buy) + ' (stop ' + str('%.2f' % trade.stop2) + ')'


def trades_week():
    trades = calc_all_trends()
    lastest = trades[-1].end

    for trade in trades:
        print str(trade.begin) + ' ' + trade.code + ' @' + str('%.2f' % trade.buy)


def day():
    jgrafixPath = r'C:\Tools\JGrafix\dados'
    import_quote_from_jgrafix(jgrafixPath)
    trades()
